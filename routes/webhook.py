...
def handle_webhook(target_dir, build_command):
    try:
        ...
        try:
            subprocess.Popen(
                f"source $HOME/.nvm/nvm.sh && {build_command}",
                shell=True,
...
def webhook_route(app):
    ...
    def webhook():
        return handle_webhook('/home/code/vitePress-Web', 'nvm use 20 && yarn docs:build')

    @app.route('/webhook-opt', methods=['GET', 'POST'])
    def webhook_opt():
        return handle_webhook('/home/cunchu/code/ibs-jeecg-vue', 'nvm use 16 && yarn build:test')
